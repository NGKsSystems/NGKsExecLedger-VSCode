...
async function executeArtifactsBundleExport(): Promise<void> {
  try {
    ...
    const exportScript = path.join(repoRoot, "tools", "export_artifacts_bundle.ps1");
    ...
    await vscode.window.withProgress({
      ...
      title: "ExecLedger: Creating artifacts bundle...",
      cancellable: false
    }, async (progress) => {
      try {
        ...
        if (bundleOkMatch && bundleOkMatch[1] === "True" && zipMatch) {
          ...
          // Refresh artifacts status bar after successful export
          onExportComplete();
